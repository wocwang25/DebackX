...
Run MyDebackX as a separate HTTP worker. The web backend should call this worker over the internal network instead of importing OCR/MT code into the web process.
...
docker run --gpus all --rm -p 8000:8000 <dockerhub-user>/iimt-worker:release
```
...
def get_translation_job(worker_url, job_id):
    response = requests.get(f"{worker_url}/jobs/{job_id}", timeout=10)
    response.raise_for_status()
    return response.json()
```
