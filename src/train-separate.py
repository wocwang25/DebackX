...
from LoadData import TextImageBackgroundDataset, get_dataloader
import torch
...
def train(step, model, train_dl, valid_dl, optimizer, lr_schedule, loss_func, accelerator, writer, save_path):
    model.train()
    each_loss = []
    for img, background, text_img in train_dl:
        output_dict = model(img)
        output_back = output_dict["back_img"]
        output_text = output_dict["text_img"]
        back_l2 = (output_back - background).pow(2).mean()
        back_lpip = loss_func["back-lpip"](output_back, background).mean()

        text_l2 = (output_text - text_img).pow(2).mean()
        text_lpip = loss_func["text-lpip"](output_text, text_img).mean()        

        loss = back_l2 + 0.1*back_lpip + text_l2 + 0.1*text_lpip
        ...
        if accelerator.is_local_main_process:
            ...
            writer.add_scalar(
                tag="train/back_l2", scalar_value=back_l2.item(), global_step=step
            )
            writer.add_scalar(
                tag="train/back_lpip", scalar_value=back_lpip.item(), global_step=step
            )
            writer.add_scalar(
                tag="train/text_l2", scalar_value=text_l2.item(), global_step=step
            )
            writer.add_scalar(
                tag="train/text_lpip", scalar_value=text_lpip.item(), global_step=step
            )
            writer.add_scalar(
...
def valid(step, model, valid_dl, loss_func, accelerator, writer):
    ...
    each_loss = []
    with torch.no_grad():
        for img, background, text_img in valid_dl:
            output_dict = model(img)
            output_back = output_dict["back_img"]
            output_text = output_dict["text_img"]
            back_l2 = (output_back - background).pow(2).mean()
            ...
            back_l2 = accelerator.gather_for_metrics(back_l2).mean().item()
            back_lpip = accelerator.gather_for_metrics(back_lpip).mean().item()
            text_l2 = accelerator.gather_for_metrics(text_l2).mean().item()
            text_lpip = accelerator.gather_for_metrics(text_lpip).mean().item()
            loss = accelerator.gather_for_metrics(loss).mean().item()

            each_back_l2.append(back_l2)
            each_back_lpip.append(back_lpip)
            each_text_l2.append(text_l2)
            each_text_lpip.append(text_lpip)
        ...
        if accelerator.is_local_main_process:
            writer.add_scalar(
                tag="valid/back_l2", scalar_value=np.mean(each_back_l2), global_step=step
            )
            writer.add_scalar(
                tag="valid/back_lpip", scalar_value=np.mean(each_back_lpip), global_step=step
            )
            writer.add_scalar(
                tag="valid/text_l2", scalar_value=np.mean(each_text_l2), global_step=step
            )
            writer.add_scalar(
                tag="valid/text_lpip", scalar_value=np.mean(each_text_lpip), global_step=step
            )
            writer.add_scalar(
...
def train_loop(config_path):
    ...
    train_src_img_path  = data_config["train_src_img_path"]
    train_tgt_img_path  = data_config["train_tgt_img_path"]
    train_src_text_img_path  = data_config["train_src_text_img_path"]
    train_tgt_text_img_path  = data_config["train_tgt_text_img_path"]
    train_background_img_path = data_config["train_background_img_path"]

    valid_src_img_path  = data_config["valid_src_img_path"]
    valid_tgt_img_path  = data_config["valid_tgt_img_path"]
    valid_src_text_img_path  = data_config["valid_src_text_img_path"]
    valid_tgt_text_img_path  = data_config["valid_tgt_text_img_path"]
    valid_background_img_path = data_config["valid_background_img_path"]
    ...
    loss_func = {"back-lpip": lpips.LPIPS(net="vgg", verbose=False).to(accelerator.device), "text-lpip": lpips.LPIPS(net="vgg", verbose=False).to(accelerator.device)}
    ...
    train_ds = TextImageBackgroundDataset(train_src_img_path, train_tgt_img_path, train_background_img_path, train_src_text_img_path, train_tgt_text_img_path)
    valid_ds = TextImageBackgroundDataset(valid_src_img_path, valid_tgt_img_path, valid_background_img_path, valid_src_text_img_path, valid_tgt_text_img_path)
