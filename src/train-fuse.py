...
def train(step, model, train_dl, valid_dl, optimizer, lr_schedule, loss_func, accelerator, writer, save_path):
    ...
    for img, background, text_img in train_dl:
        output_dict = model(background, text_img)
        output_img = output_dict["img"]

        l2 = (output_img - img).pow(2).mean()
        lpip = loss_func["lpip"](output_img, img).mean()
        loss = l2 + 0.1*lpip
...
def valid(step, model, valid_dl, loss_func, accelerator, writer):
    ...
    with torch.no_grad():
        ...
        if accelerator.is_local_main_process:
            writer.add_scalar(
                tag="valid/back_l2", scalar_value=np.mean(each_l2), global_step=step
            )
            writer.add_scalar(
                tag="valid/back_lpip", scalar_value=np.mean(each_lpip), global_step=step
            )
            writer.add_scalar(
